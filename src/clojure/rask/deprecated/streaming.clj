(ns rask.deprecated.streaming
  (:refer-clojure :exclude [filter iterate map max min print reduce])
  (:require [rask.deprecated.util :as util]
            [rask.deprecated.type :as type])
...
(defn join
  ...
  (let [k1 (reify
             ...
             (getKey [_ x]
               (left-key x))
             ResultTypeQueryable
             (getProducedType [_] (.getTypeInfo ^TypeHint (type/type-info Object))))
        k2 (reify
             KeySelector
             (getKey [_ x]
               (right-key x))
...
(defn ^KeyedStream key-by
  ...
  (cond
    ...
    (let [f (reify KeySelector
      ...
      (KeyedStream. stream f (.getTypeInfo ^TypeHint (type/type-info Object))))
